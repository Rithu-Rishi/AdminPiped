...
export const getAllSubPrograms = async ({ page = 1, per_page = 10, search = "" }) => {
  try {
    const response = await API.get(`/api/sub-programs?page=${page}&per_page=${per_page}&search=${search}`);
...
export const updateSubProgram = async (id, formData) => {
  try {
